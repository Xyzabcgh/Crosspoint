...
# Modify the combined_file_generator function to use the new dialog
def combined_file_generator(input_folder, output_folder, gui):
    """gui.show_content("Combined File Generation", "Processing BOM files for combined mode...")
    ...
    for file_name in input_files:
        ...
        try:
            ...
            # Use the new dialog to collect column numbers
            part_desc_col, mfg_part_num_col, qty_col,all_value_col,source_col= ask_columns_dialog()
            #print("hi")
            output_df = pd.DataFrame()
            output_df['Part Description'] = df_bom.iloc[0:, part_desc_col]
            output_df["Manufacturer's Part Number"] = df_bom.iloc[0:, mfg_part_num_col]
            output_df["Quantity"] = df_bom.iloc[0:, qty_col].astype(float)
            output_df["All value"] = df_bom.iloc[0:, all_value_col]
            output_df["Source"] = df_bom.iloc[0:, source_col]
            ...
            output_df = output_df.groupby(['Part Description', "Manufacturer's Part Number"], as_index=False).apply(lambda group: pd.Series({'Final QTY': group['Final QTY'].sum(),'Projects': ', '.join(set(group['Projects'].astype(str))),
            'All value': group['All value'].min() if group['All value'].notna().any() else None,
            'Source': group.loc[group['All value'].idxmin(), 'Source'] if group['All value'].notna().any() else None})).reset_index(drop=True)
            #output_df.to_excel(f"{output_folder}/{file_name}_BOM.xlsx",index=False)
...
# Function for Customer File Generator (Placeholder)
"""def customer_file_generator(input_folder, output_folder, gui, process_loss_factors):
    ...
    gui.show_content("Customer File Generation", "Processing BOM files for customer mode...")
    input_files = [f for f in os.listdir(input_folder) if f.endswith('.xlsx')]
   ...
   # mapping_df = external_df[["Manufacturer's Part Number", "Type", "Pin Count"]]
    ...
    for file_name in input_files:
        ...
        try:
            ...
            # Merge with external file for Type and Pin Count
            #mapping_df.columns = ["Manufacturer's Part Number", 'Type', 'Pin Count']
            #output_df = output_df.merge(mapping_df, on="Manufacturer's Part Number", how="left")

            # Merge assembly data with output_df for Type and Pin Count
            #df_assembly = pd.merge(df_assembly, output_df[['Manufacturer\'s Part Number', 'Type', 'Pin Count']], on="Manufacturer's Part Number", how="left")

            # Ensure 'Type' and 'Pin Count' are present; if not, assign default values
            #df_assembly['Type'].fillna('Unknown', inplace=True)
            #df_assembly['Pin Count'].fillna(0, inplace=True)

            # Calculate "Per Board Count"
            #df_assembly['Per Board Count'] = df_assembly['Quantity'] * df_assembly['Pin Count']

            # Clean up by dropping extra columns from output_df
            #output_df = output_df.drop(columns=["Type", "Pin Count"])
            ...
            output_df.to_excel(os.path.join(output_folder, f"{project_name} Customer BOM{timestamp}.xlsx"), index=False)

            # Save the updated Assembly sheet to an Excel file
