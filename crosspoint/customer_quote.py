...
def safe_convert_to_numeric(value):
    try:
        ...
        # Try converting to a numeric type (int or float)
...
input_base_dir = "Customers"
output_base_dir = "Output"
source_mapping_file = "D:\\crosspoint\\output.csv"  # Shared source mapping file location
column_mapping_file = "D:\\crosspoint\\template.xlsx"  # Column mapping file location
...
try:
    ...
    logger.error(f"Error loading source mapping file: {e}")
    exit(1)
...
try:
    ...
    logger.error(f"Error loading column mapping file: {e}")
    exit(1)
...
for customer_name in os.listdir(input_base_dir):
    ...
    if not os.path.isfile(bom_file):
        logger.warning(f"Missing BOM file for {customer_name}. Skipping...")
        continue
    
    try:
        bom_df = pd.read_excel(bom_file)
        logger.info(f"Loaded BOM sheet for {customer_name}")
    except Exception as e:
        logger.error(f"Error reading BOM file for {customer_name}: {e}")
        continue
    ...
    for vendor_file in os.listdir(customer_dir):
        if "_Quote.xlsx" in vendor_file:
            # Extract vendor name from filename format (before the first '_')
            vendor_name = vendor_file.split("_")[0]
            vendor_path = os.path.join(customer_dir, vendor_file)
            try:
                vendor_df = pd.read_excel(vendor_path)
                logger.info(f"Loaded quote file for vendor: {vendor_name}")
                
                # Get the mapped column numbers for this vendor
                if vendor_name in column_mapping_df.columns:
                    ...
                    logger.warning(f"Vendor {vendor_name} not found in column mapping file.")
            except Exception as e:
                logger.error(f"Error reading vendor file {vendor_file} for {customer_name}: {e}")
                continue
    ...
    # Debugging: Check BOM before merging
    logger.info(f"BOM Data before merging: {bom_df[['Manufacturer Part Number']].head()}")

    for vendor, df in vendor_quotes.items():
        # Remove duplicates in vendor quotes
        ...
        logger.info(f"After merging with {vendor}, BOM columns: {bom_df.columns}")

    # Check the BOM DataFrame after all merges
    logger.info(f"BOM Data after all merges: {bom_df.head()}")

    # Now, determine final price and source
    final_prices = []
    sources = []
    for index, row in bom_df.iterrows():
        ...
        # Get the price from the preferred vendor's column, if available
        if preferred_vendor:
            price_column = f"{preferred_vendor}_Price"
            price = row.get(price_column)  # Safely get price
        else:
            price = None
        ...
        # Debugging: Log the part number, vendor, and price
        logger.info(f"Part Number: {part_number}, Preferred Vendor: {preferred_vendor}, Price: {price}")
    
    # Add final Price and Source columns to the BOM DataFrame
    bom_df["Price"] = final_prices
    bom_df["Source"] = sources
    
    # Debugging: Check final prices before saving
    logger.info(f"BOM Data with final prices: {bom_df[['Manufacturer Part Number', 'Price', 'Source']].head()}")
    ...
    try:
        ...
        logger.info(f"Updated BOM sheet saved for {customer_name} at {output_path}")
    except Exception as e:
        logger.error(f"Error saving output file for {customer_name}: {e}")
"""
